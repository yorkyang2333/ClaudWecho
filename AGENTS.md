...
2. **Progress Bar Seeking (`PlayerScreen`)**:
   ...
   - For OPPO Watch crown seeking, MUST accumulate `vScroll` across continuous events (`accumulatedOppoScroll`) with a threshold (`threshold = 25.0f`) and reset after 300ms inactivity to avoid over-sensitivity from accidental touches.
